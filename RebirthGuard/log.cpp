...
VOID RG_Report(DWORD flag, RG_REPORT_CODE code, PVOID data1, PVOID data2)
{
	...
	{
		FILE* log = NULL;
		fopen_s(&log, RGS("RebirthGuard.log"), "a+");
		fprintf(log,
			RGS("[%04d-%02d-%02d %02d:%02d:%02d] Pid : %d / Code : 0x%08X / %S 0x%p / %S 0x%p\n"),
	...
	if (flag & RG_ENABLE_POPUP)
	{
		...
		RG_strcat(scriptpath, RGS("\\RebirthGuard.vbs"));

		FILE* log = NULL;
		fopen_s(&log, RGS("RebirthGuard.vbs"), RGS("w+"));
		fprintf(log,
			RGS("Dim obj, file\n"
